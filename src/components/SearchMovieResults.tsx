...
const SearchMovieResults = ({ posterPath, id, title }: any) => {
  ...
  return (
    <>
      <div className="block w-full  group md:mr-4 p-2 py-4  relative group/item">
        <Link to={`/browse/${id}`}>
          <img
            src={Movie_Logo_URL + posterPath}
            alt="trending"
            className="w-96 h-full group-hover/item:opacity-30 group-hover/item:transition-all  group-hover/item:duration-500 group-hover/item:ease-in-out"
          />
        </Link>
